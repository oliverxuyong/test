...
function showAllPosters(msg) {
	...
	var mytopicId=msg.tmp_topicid;//msg_type为6时，需要我的topicId来打开聊天窗口，统一用tmp_topicId，普通窗口时topicid=tmp_topicid 2017/03/20 郑
	...
	for (j in msgArr) {//遍历每一条消息.
		...
		//console.log("测试："+msgType);
		if ((msgType == 0 || msgType == 2 || msgType == 3 || msgType == 4 || msgType == 1|| msgType == 5 || msgType==6) && sender_topicid != topicId) {//由于本人发言的msgType=0,所以需要加一个非本人判断(也可用senderId != userId)
		...
		//叶夷  2017.06.15  -->msg_type:6=有过往发言的消息   
		if ((msgType == 0 || msgType == 2 || msgType == 3 || msgType == 4 || msgType == 1|| msgType == 5 || msgType == 6) && sender_topicid != topicId) {//由于本人发言的msgType=0,所以需要加一个非本人判断(也可用senderId != userId)
			//对方的直接发言0,对方回复我的发言3,或者是推荐(包括已接受的推荐).这时可以将之屏蔽或新窗.屏蔽推荐表示不喜欢这个推荐,以后不要再发了(后台是否处理了这个动作?).
			//2017.07.07  叶夷   为了让不同域名的管理员头像不能点击，所以用senderId=1来过滤
		...
		if(msgType == 6){
			//var mytopicId=msg.mytid;//plan 1： 通过后台传回原先的topicId，msgType=6时才有这个参数 zheng, 2017.03.20
			...
			histBtn.click(function(){
				...
				//console.log("senderId: "+senderId+"sender_topicid: "+sender_topicid);
				//execRoot("getUserReqTopicMessages('"+senderId+"','"+sender_topicid+"','"+mytopicId+"')");
				execRoot("getUserReqTopicMessages('"+senderId+"','"+sender_topicid+"','"+tmpTopicId+"')");//直接使用tmpTopicId，dialog_page,dialog_page_newtopic,dialog_page_movetonewtopic通用
			});
...
function processDescendantConnectData(_msg){
	...
	$.each(_list,function(i,item){
		result.push('<div style="float: left;width: 100%;display: inline-flex;margin-top:15px;" onclick="triggerClick(\'checkbox\',this)">');
		result.push('<div style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;color: #999;z-index:-1;">');
...
function descendantfinish(){
	...
	if(_checkRadio=="no") {
		...
		if (_obj.size() == 0) {
			...
			//叶夷   2017.06.15    对邀请功能的话题名增加特殊字符过滤方法excludeSpecial()
			execRoot("createNewTopic2AppointedTopics('" + topicId + "','" + userId + "','" + excludeSpecial($("#pop_topicName").val()) + "','" + _checkRadio + "','" + v_topicid + "')");
		}
	}else{
		//execRoot("createNewTopic2AppointedTopics('" + topicId + "','" + userId + "','" + $("#pop_topicName").val() + "','" + _checkRadio + "','')");
		//叶夷   2017.06.15    对邀请功能的话题名增加特殊字符过滤方法excludeSpecial()
		execRoot("createNewTopic2AppointedTopics('" + topicId + "','" + userId + "','" + excludeSpecial($("#pop_topicName").val()) + "','" + _checkRadio + "','')");
	}
}
...
function processDescendantData(_msg){
	...
	//叶夷   2017.06.15    对邀请功能的话题名增加特殊字符过滤方法excludeSpecial()
	var _topictitle = excludeSpecial($("#pop_topicName").val());
	closePop();
	var contextresult = [];
	contextresult.push('<div id="entrytopic">');
...
function RelationConstr(){
	...
	this.relation_pop=function(){
		...
		/*start:叶夷
		解决问题：之前点击全选“框”没有效果不会√，之后在<input>中添加onclick="relation.checkAllBox()"，问题解决*/
		contextresult.push('<div class="check-div" onclick="relation.checkAllBox()"><input type="checkbox" id="topicall" onclick="relation.checkAllBox()"/><span>全选</span></div>');
		/*end:叶夷*/
		...
		contextresult.push('</div>');
		contextresult.push('</div>');
		alertWin(contextresult.join(''),'与'+'[ ' + cutStringIfTooLong(title,9) + ' ]'+'连接的话题',_w,_h);
	...
	this.processRelationData = function(_msg){
		...
		var result = [];
		$.each(_list,function(i,item){
			result.push('<div style="float: left;width: 100%;display: inline-flex;margin-top:15px;line-height:24px;">');
			...
			if(isqingting == 'no' || isqingting == '') {
				result.push('<div style="background:url(../image/titbg.gif) center top repeat-x;box-shadow:0px 2px 4px rgba(34, 25, 25, 0.2);padding: 1px 2px;margin:1px;font-size:9px;border-radius: 4px;cursor:pointer;font-size:12px;width:25px;height:20px;line-height:20px;text-align:22px;" onclick="relation.personalDialog(this)" topicId="'+item.topicid+'" userId="'+item.userid+'" userName="'+item.username+'">单聊</div>');
			}
			result.push('</div>');
		});
		var _obj = $(".more-rule");
		...
		if(_totalcount == 0){
			...
			if(_show==_totalcount){
				_obj.removeAttr("onclick");
				_obj.html("["+_show+"/"+_totalcount+"]");
				_obj.css('cursor','default');
			}else{
				connpage += conncount;
				_obj.css('cursor','point');
			}
		}
	};
	//加载更多数据
...
function showUserReqTopicMessages(receivedData){
	...
	if(undefined == _poster_arr || _poster_arr.length == 0 ){
		...
		$.each(_poster_arr, function (i, item) {
			 ...
			 *   修改过往发言内容列表的样式
			 */
			contextResult.push('<time class="p-send-time"> ' + item.datetime_str + '</time>');
			contextResult.push('<div class="p-user p-other">');
			...
			contextResult.push('<div class="p-detail ">'+item.poster+'</div></div>');
			/*end: 叶夷
			 */
		});
	}
