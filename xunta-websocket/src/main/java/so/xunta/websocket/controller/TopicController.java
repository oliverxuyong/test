...
public class TopicController {
	...
	public void createTopic(WebSocketSession session, TextMessage message) {
		...
		logger.debug("接收发起群聊话题的信息解析:topic_name=" + topic_name + " creator_uid=" + creator_uid + " user_ids=" + user_ids
				+ " create_datetime=" + create_datetime + " end_datetime" + end_datetime);
		...
		User creator_user = userService.findUser(Long.valueOf(creator_uid));
		String username = creator_user.getName();
		String chatmsg_content = username + "创建了群聊话题[" + topic_name + "]";
	...
	public void getAllTopic(WebSocketSession session, TextMessage message) {
		...
		for (Topic topic : topicList) {
			...
			if (!userType.equals("REJECT")) {
				...
				// 获得话题的未读消息数
				List<TopicHasUnreadMsgNum> topicHasUnreadMsgNumArray = topicHasUnreadMsgNumService
						.findUserHasUnreadMsgTopicByUserid(Long.valueOf(userid));
				...
				/*
				 * {"to_user_id":"", "msg":"", "create_time":"", "ifread":"已读",
				 * "to_user_name":"", "to_user_imgUrl":""},
				 */
	...
	private void sendMsgs(List<WebSocketSession> receivers, String topic_id, String topic_name, String send_uid,
			String send_name, String send_userImage, String chatmsg_content, String chatmsg_id, String handle,
			String create_datetime, String type) {
		logger.debug("在线普通发言");
		logger.debug("在线 人数" + receivers.size());
		// 遍历每个receivers
		for (int i = 0; i < receivers.size(); i++) {
			WebSocketSession user = receivers.get(i);
			JSONObject chatmsgReturnJSON = buildChatmsgReturnJSON(topic_id, topic_name, send_uid, send_name,
					send_userImage, chatmsg_content, chatmsg_id, handle, create_datetime, type);
			logger.debug("在线发送消息的json数据：" + chatmsgReturnJSON.toString());
			socketService.chat2one(user, chatmsgReturnJSON);
		}
	}

	private JSONObject buildChatmsgReturnJSON(String topic_id, String topic_name, String send_uid, String send_name,
			String send_userImage, String chatmsg_content, String chatmsg_id, String handle, String create_datetime,
			String type) {
		JSONObject chatmsgReturnJSON = new JSONObject();
		chatmsgReturnJSON.put("_interface", "1117-2");
	...
	private void sendofflineMsgs(String userid, List<Long> offlineUserids, String content, String username,
		...
		for (Long touserid : offlineUserids) {
			...
			if (toopenid != null && !toopenid.equals("") && !toopenid.equals("\"\"")) {
				...
				logger.debug("模版消息信息通过usergroup获取：usergroup=" + usergroup + " templateid=" + templateid
						+ " templateurl=" + templateurl + " appid=" + appid + " appsecret=" + appsecret);
